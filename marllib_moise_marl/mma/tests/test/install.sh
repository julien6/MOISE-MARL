...
if [ -f "$HOME/miniconda3/etc/profile.d/conda.sh" ]; then
    echo -e "\n\nINSTALLATION ON COMPUTATIONAL REMOTE SERVER\n"
    ssh soulej@bigfoot.ciment -t "cd /bettik/soulej ; rm -rf omarl_experiments ; git clone https://github.com/julien6/omarl_experiments.git ; cd omarl_experiments ; git checkout test ; cd mma_wrapper/tests/test"
    rsync -avxH tmp_install.sh soulej@bigfoot.ciment:/bettik/soulej/omarl_experiments/mma_wrapper/tests/test
    rm -rf tmp_install.sh
    ssh soulej@bigfoot.ciment -t "cd /bettik/soulej/omarl_experiments/mma_wrapper/tests/test ; ./tmp_install.sh ; rm -rf tmp_install.sh ; echo -e \"\n\nINSTALLATION FINISHED!\n\""
fi
